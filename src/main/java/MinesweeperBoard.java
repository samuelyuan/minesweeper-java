public class MinesweeperBoard extends Board {
	...
	public boolean getIsRevealed(int r, int c) {
		return isRevealed[r][c];
	}

	public boolean getIsFlagged(int r, int c) {
		return isFlagged[r][c];
	}

	public void setIsRevealed(boolean flag, int r, int c) {
		isRevealed[r][c] = flag;
	}

	public void setIsFlagged(boolean flag, int r, int c) {
		isFlagged[r][c] = flag;
	}
	...
	public void setNumMinesLeft(int numMinesLeft) {
		this.numMinesLeft = numMinesLeft;
	}
	...
	public boolean isLose() {
		for (int r = 0; r < getNumRows(); r++) {
			for (int c = 0; c < getNumCols(); c++) {
				if (isRevealed[r][c] && getTile(r, c) == ID_MINE) {
					return true;
				}
			}
		}
	...
	public boolean isWin() {
		...
		for (int r = 0; r < getNumRows(); r++) {
			for (int c = 0; c < getNumCols(); c++) {
				...
				if (isRevealed[r][c] && getTile(r, c) >= MinesweeperBoard.ID_EMPTY) {
					numNonMinesRevealed++;
				}
			}
		}

		return numMinesHidden + numNonMinesRevealed == getNumRows() * getNumCols();
	}
	...
	public void floodFillEmptyTiles(int centerRowNum, int centerColNum) {
		...
		if (getTile(centerRowNum, centerColNum) != ID_EMPTY && !isFlagged[centerRowNum][centerColNum]) {
			isRevealed[centerRowNum][centerColNum] = true;
		}

		if (!isRevealed[centerRowNum][centerColNum] && !isFlagged[centerRowNum][centerColNum]) {
			isRevealed[centerRowNum][centerColNum] = true;
		} else {
			return;
		}
